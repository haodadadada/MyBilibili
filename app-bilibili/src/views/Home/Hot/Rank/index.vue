<template>
    <div class="container-rank">
        <header class="rank-tab-wrap mb-10">
            <ul class="rank-tab flex-start text-[13px]">
                <li class="mr-5" :class="activeTab.index === index ? 'active-tab' : ''" v-for="(tab, index) of rankTabs" :key="tab.tid" @click="updateRankVideos(tab, index)">
                    {{ tab.name }}
                </li>
            </ul>
        </header>
        <section class="cards-content" v-if="!isDrama && !isLoading">
            <div v-for="(item, index) in rankVideos" :key="item.aid" class="cards-wrapper" @click="handleClickVideo(item, index)">
                <div class="box-card flex-column-center">
                    <div class="card-content mb-10">
                        <div class="card-cover">
                            <img alt="" class="img" v-lazyload="item.pic" />
                            <i class="num" :class="'num-' + (index + 1)">
                                <svg viewBox="0 0 41 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="rank-icon">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0H41C36.4908 2.23529 33.1107 6.24084 31.6656 11.0618L27.6131 24.5817C26.6491 27.7975 23.6896 30 20.3324 30H0V0Z" :fill="'url(#rank-linear-' + (index + 1) + ')'"></path> 
                                    <defs>
                                        <linearGradient :id="'rank-linear-' + (index + 1)" x1="0" y1="0" x2="0" y2="30" gradientUnits="userSpaceOnUse">
        ...
        <section class="cards-content" v-if="isDrama && !isLoading">
            <div v-for="(item) in rankVideos" :key="item.season_id" class="cards-wrapper">
                <div class="box-card flex-column-center">
                    <div class="card-content mb-10">
                        <div class="card-cover">
                            <img alt="" class="img" v-lazyload="`${item.ss_horizontal_cover}@412w_232h_1c_!web-popular.avif`" />
                            <i class="num" :class="'num-' + item.rank">
                                <svg viewBox="0 0 41 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="rank-icon">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0H41C36.4908 2.23529 33.1107 6.24084 31.6656 11.0618L27.6131 24.5817C26.6491 27.7975 23.6896 30 20.3324 30H0V0Z" :fill="'url(#rank-linear-' + item.rank + ')'"></path> 
                                    <defs>
                                        <linearGradient :id="'rank-linear-' + item.rank" x1="0" y1="0" x2="0" y2="30" gradientUnits="userSpaceOnUse">
                                            <stop class="start"></stop> 
                                            <stop offset="1" class="end"></stop>
                                        </linearGradient>
                                    </defs>
                    ...
                    <div class="card-info">
                        ...
                        <div class="card-info-bottom flex-start">
                            <div class="card-info-owner flex-start">
                                <span class="card-info-bottom-author">{{ item.desc }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
<script setup lang="ts">
    ...
    import rankTabs from './hotRankTabs';
    ...
    import { getHomeRank, getHomePgcRank } from '@/api/home/hot';
    import { getHomePlayUrl } from '@/api/home';

    interface RankVideoItem {
        aid: number;
        bvid: string;
        cid: number;
        title: string;
    ...
    let activeTab = ref({index: 0, tid: rankTabs[0].tid});
    const updateActiveTab = (tid: number, index: number) => {
        activeTab.value.tid = tid;
        activeTab.value.index = index;
    };
    const updateRankVideos = async (tab: { name: string, season_type?: number, tid: number }, index: number) => {
        if(index === activeTab.value.index) return;
        isLoading.value = true;
        if(tab.season_type) {
            await fetchHomePgcRankList(tab.season_type, sessdata);
        }
        else {
            await fetchHomeRankList(tab.tid, sessdata);
        };
        updateActiveTab(tab.tid, index);
        isLoading.value = false;
    };
    let rankVideos = ref<RankVideoItem[]>([]);
    ...
    const fetchHomeRankList = async (tid: number, sessdata: string = ''): Promise<void> => {
        ...
        const response = await getHomeRank(completeParamsObj, sessdata);
        const successAction = R.pipe(
            R.path(['data', 'list']) as (data: any) => RankVideoItem[],
            R.tap(list => list.length > 50 ? rankVideos.value = list.splice(0, 50) : rankVideos.value = list),
        );
    ...
    const fetchHomePgcRankList = async (season_type: number, sessdata: string = '') => {
        ...
        // 将 URLSearchParams 转换为对象
        const paramsObj = Object.fromEntries(newParams.entries());
        if(!paramsObj.season_type) return null;
        const completeParamsObj = {
    ...
    const fetchHomeRankPlayurl = R.curry(async (item, sessdata) => {
        const {bvid, cid} = item;
    ...
    const handleMouseEnter = async (item: RankVideoItem, index: number) => {
        curMouseIndex.value = index;
        playVideo(item, index);
    };
    
    const playVideo = debounce(async (item, index) => {
        const videosStreamInfo = await fetchHomeRankPlayurl(item, sessdata);
        if(!videosStreamInfo) return;
        await fetchVideoStream(videosStreamInfo, index, rankVideos.value[index], videoRefs, videoBlobUrls, curMouseIndex);
    });

    const stopPlayPreview = (item: RankVideoItem, index: number): void => {
    ...
    const openPlayerWindow = (videoItem: RankVideoItem, index: number, routePath: string): void => {
    ...
    const handleClickVideo = (videoItem: RankVideoItem, index: number) => {
        openPlayerWindow(videoItem, index, route.path);
        updateCurPlayerWindowIndex(index);
        stopPlayPreview(videoItem, index);
    };

    const handleMouseLeave = (item: RankVideoItem, index: number) => {
        const videoRef = R.pathOr(null, [index], videoRefs.value);
        if(!videoRef) return;
        stopPreview(item, videoRef, playVideo, curMouseIndex);
    };

    const resetVideos = (videos: RankVideoItem[]) => {
        if(!videos) return [];
    ...
    onMounted(async () => {
        await fetchHomeRankList(activeTab.value.tid, sessdata);
        isLoading.value = false;
    });
    ...
    onDeactivated(() => {
        ...
        rankVideos.value = resetVideos(rankVideos.value);
    });
</script>
                ...
                .box-card {
                    ...
                    .card-content {
                        ...
                        .card-cover {
                            ...
                            .rank-list .rank-item .content .img .num-3 .start {
                                stop-color: #ffe4c7
                            }

                            .rank-list .rank-item .content .img .num-3 .end {
                                stop-color: #ed9487
                            }
                            .card-mask {
                                position: absolute;
