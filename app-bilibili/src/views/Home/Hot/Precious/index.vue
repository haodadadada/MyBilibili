<template>
    <div class="container-precious" v-if="!isLoading">
        <div class="content-precious">
            <div class="cards-header flex-center">
                <div class="precious-header flex-center">
            ...
            <div v-for="(item, index) in preciousVideos" :key="item.aid" class="item-content" @click="handleClickVideo(item, index)">
                <div class="box-card flex-column-center">
                    ...
                    <div class="card-info">
                        ...
                        <div class="card-reason mt-5 flex-center flex-start">
                            <component :is="preciousTitleIcon" class="mr-5"></component>
                            <span class="text-[12px]">{{item.achievement}}</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="no-more text-[12px]">陆续更新中, 敬请期待</div>
    </div>
</template>
...
<script setup lang="ts">
    ...
    import { getHomePrecious } from '@/api/home/hot';
    import { getHomePlayUrl } from '@/api/home';
    import preciousTitleIcon from '@/assets/home/precious_icon.svg';

    interface PreciousVideoItem {
        achievement: string;
        aid: number;
        bvid: string;
        cid: number;
    ...
    let preciousVideos = ref<PreciousVideoItem[]>([]);
    const fetchHomePrecious = async () => {
        const response = await getHomePrecious();
        const successAction = R.pipe(
            R.path(['data', 'list']) as (data: any) => PreciousVideoItem[],
            R.tap(list => preciousVideos.value = list)
        );
    ...
    const playVideo = debounce(async (item, index) => {
        const videosStreamInfo = await fetchHomePreciousPlayurl(item, sessdata);
        if(!videosStreamInfo) return;
        await fetchVideoStream(videosStreamInfo, index, preciousVideos.value[index], videoRefs, videoBlobUrls, curMouseIndex);
    });

    const stopPlayPreview = (item: PreciousVideoItem, index: number): void => {
    ...
    const openPlayerWindow = (videoItem: PreciousVideoItem, index: number, routePath: string): void => {
    ...
    const handleClickVideo = (videoItem: PreciousVideoItem, index: number) => {
        openPlayerWindow(videoItem, index, route.path);
        updateCurPlayerWindowIndex(index);
        stopPlayPreview(videoItem, index);
    };

    const handleMouseEnter = async (item: PreciousVideoItem, index: number) => {
        curMouseIndex.value = index;
        playVideo(item, index);
    };

    const handleMouseLeave = (item: PreciousVideoItem, index: number) => {
        stopPlayPreview(item, index);
    };

    const resetVideos = (videos: PreciousVideoItem[]): PreciousVideoItem[] => {
        if(!videos) return [];
    ...
    onDeactivated(() => {
        ...
        preciousVideos.value = resetVideos(preciousVideos.value);
    });
</script>
            ...
            .item-content {
                ...
                .box-card {
                    ...
                    .card-info {
                        ...
                        .card-reason {
                            padding: 3px 5px;
                            color: var(--hot-precious-history-text);
                            background-color: var(--hot-precious-history-bg);
                            max-width: 100%;
                            span {
                                white-space: nowrap;
